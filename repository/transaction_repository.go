...
func (t *transactionRepo) AdjustBalance(ctx *gin.Context, creditTransaction *entities.Transaction, dbTxn *gorm.DB) error {
	...
	result := dbTxn.Where("account_id = ? AND balance < 0", creditTransaction.AccountID).Find(&transactions)
	if result.Error != nil {
		logger.Errorf("error while fetching transactions - %v", result.Error)
		return result.Error
	}

	balanceAdjustment := creditTransaction.Amount
	...
	// update the balance of the credit transaction
	if balanceAdjustment > 0 {
		creditTransaction.Balance = balanceAdjustment
		if err := t.updateBalance(ctx, dbTxn, *creditTransaction); err != nil {
			return err
		}
	}
	return nil
